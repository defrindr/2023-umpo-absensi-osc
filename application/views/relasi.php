...
<body class="bg-cream-1" style="height:100vh;">
    ...
    <div class="modal fade" id="modal-mahasiswa" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <h1 class="modal-title fs-5" id="modal-mahasiswa-label">Data Mahasiswa</h1>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <div class="mb-2">
                        <label for="search-nim" class="form-label">NIM</label>
                        <label for="search-nim" class="form-counter float-end"></label>
                        <input type="text" class="form-control" id="search-nim" maxlength="16">
                    </div>
                        ...
                        <div class="card shadow border-top-success">
                            <div class="card-body">
                                ...
                                <div class="vstack gap-2">
                                    <div>
                                        <label for="create-kelas" class="form-label">Kelas</label>
                                        <select class="form-select" id="create-kelas">
                                            <option value="0">Pilih Kategori</option>
                                        </select>
                                    </div>
                                    <div>
                                        <label for="create-dosen" class="form-label">Dosen</label>
                                        <select class="form-select" id="create-dosen">
                                            <option value="0">Pilih Kategori</option>
                                        </select>
                                    </div>
                            ...
                            <div class="card-body">
                                ...
                                <div class="vstack gap-2">
                                    ...
                                    <div>
                                        ...
                                        <select class="form-select" id="update-kelas">
                                            <option value="0">Pilih Kategori</option>
                                        </select>
                                    </div>
                                    <div>
                                        <label for="update-dosen" class="form-label">Dosen</label>
                                        <select class="form-select" id="update-dosen">
                                            <option value="0">Pilih Kategori</option>
                                        </select>
                                    </div>
                                    <div class="d-flex justify-content-end">
...
<script>
    ...
    global_category = [];
    ENDPOINTS = {
        'update': 'api/v1/relasi/update',
        'create': 'api/v1/relasi/create',
        'delete': 'api/v1/relasi/delete',
        'load': 'api/v1/relasi/get',
        'data_kelas': 'api/v1/kelas/get',
        'data_dosen': 'api/v1/dosen/get',
        'searchnim': 'api/v1/mahasiswa/searchnim',
        'get_in': 'api/v1/mahasiswa/get_in',
    }
    ...
    $('#update-submit').click(function () {
        ...
        data = {
            id: $('#update-id').val(),
            id_kelas: $('#update-kelas').val(),
            id_dosen: $('#update-dosen').val(),
        }
    ...
    $('#create-submit').click(function () {
        data = {
            id_kelas: $('#create-kelas').val(),
            id_dosen: $('#create-dosen').val(),
        }
    ...
    var table = new Tabulator("#data-table", {
        ...
        columns: [
            ...
            { title: "Dosen", field: "nama_dosen" },
            //hidden created
            { title: "Action", field: "action", width: 150, formatter: "html", tooltip: false, download: false },
        ],
    });

    var table_mahasiswa = new Tabulator("#data-mahasiswa", {
        layout: "fitColumns",      //fit columns to width of table
        responsiveLayout: "hide",  //hide columns that dont fit on the table
        addRowPos: "top",          //when adding a new row, add it to the top of the table
        history: true,             //allow undo and redo actions on the table
            ...
            success: function (data) {
                ...
                $('#loader-container').fadeOut(anim_delay, function () {
                    ...
                    $.each(data, function (i, v) {
                        ...
                        action += '<button class="btn btn-sm btn-outline-dark" onclick="viewMahasiswa(`' + id + '`)"><i class="bi bi-person"></i></button>';
    ...
    $(document).ready(function () {
        load_data();
        fill_select(global_defaults.server_url + ENDPOINTS.data_kelas, ['#create-kelas', '#update-kelas'], field = 'nama');
        fill_select(global_defaults.server_url + ENDPOINTS.data_dosen, ['#create-dosen', '#update-dosen'], field = 'nama');
    });
    ...
    function viewMahasiswa(idr) {
        current_relasi = idr;
        $('#modal-mahasiswa').modal('show');

        $.ajax({
            url: global_defaults.server_url + ENDPOINTS.get_in,
            type: 'POST',
            data: {
                //ids from list_mahasiswa
                ids: table.getRow(idr).getData().list_mahasiswa
            },
            async: false,
            dataType: 'json',
            success: function (data) {
                ...
                $.each(data, function (i, v) {
                    var id = data[i].id;
                    var action = '<div class="d-flex justify-content-between align-items-center">';
                    action += '<button class="btn btn-sm btn-outline-danger" onclick="deleteMahasiswa(`' + idr + '`,`' + id + '`)"><i class="bi bi-trash"></i></button>';
                    action += '</div>';
                    data[i].action = action;
                });

                table_mahasiswa.setData(data);
    ...
    function deleteMahasiswa(relasi, mahasiswa) {
        list_mahasiswa = table.getRow(relasi).getData().list_mahasiswa;

        //remove mahasiswa from list, split by + then join by +
        list_mahasiswa = list_mahasiswa.split(' ').filter(function (value, index, arr) {
        ...
        $.ajax({
            ...
            success: function (data) {
                load_data();
                viewMahasiswa(relasi);

            }
        });
    }

    function addMahasiswa(relasi, mahasiswa) {
        list_mahasiswa = table.getRow(relasi).getData().list_mahasiswa;
    ...
    $('#search-nim').keypress(function (e) {
        if (e.which == 13) {
            $.ajax({
                url: global_defaults.server_url + ENDPOINTS.searchnim,
                type: 'POST',
                data: {
                    nim: $('#search-nim').val()
                },
                success: function (data) {
                    ...
                    $('#search-nim').val('');
                    addMahasiswa(current_relasi, data.data.id);
                }
            });
        }
    });
